...
#define _hash_set_op(op)             (((uint64_t)op & 0xFFFFFFFF) << 8)
...
int ovx_controller_read_attr(vsi_nn_node_t* node,
  ...
  switch (hash) {
    ...
    case ATTR_CONV_RELU_POOL_STRIDE_CONV:
      node->nn_param.conv2d.stride[0] = shape[0];
      node->nn_param.conv2d.stride[1] = shape[1];
      break;
    //case ATTR_POOL_STRTIDE:
    case ATTR_CONV_RELU_POOL_STRIDE_POOL:
      node->nn_param.pool.stride[0] = shape[0];
      node->nn_param.pool.stride[1] = shape[1];
      break;
    case ATTR_CONV_RELU_POOL_KSIZE_POOL:
      node->nn_param.pool.ksize[0] = shape[0];
      node->nn_param.pool.ksize[1] = shape[1];
      break;
